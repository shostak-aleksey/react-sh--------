...
export const getUsers = (pageSize, users) => {
	return dispatch => {
		dispatch(toggleDownloadStatus(true))
		userApi.getPageSize(pageSize, users).then(response => {
			dispatch(setState(response.data.items))
			dispatch(toggleDownloadStatus(false))
		})
	}
}
export const setSelectPage = page => ({ type: SET_SELECT_PAGE, page })
export const toggleDownloadStatus = status => ({
	type: TOGGLE_DOWNLOAD_STATUS,
	status,
})
